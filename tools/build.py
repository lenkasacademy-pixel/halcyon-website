...
SITE = {
    ...
    # The telecallers' CRM. It is the same Apps Script deployment answering at
    # ?app=crm, and it cannot be hosted here: its server half needs Apps Script's
    # own access to the leads sheet. /crm is only a doorway — see write_htaccess.
    'crm_url': 'https://script.google.com/macros/s/AKfycbyZN6HDK5kDK0jB5hvpHNOmhlRQF452Ulf2rRN5owK1zutVenDAET2a61kQu1QZ4zfO4g/exec?app=crm',
...
MOBILE_NAV_CSS = r"""
/* ---------------------------------------------------------------- menu ----
   Appended to every page by tools/build.py, after the page's own nav rules,
   so the `.nav__links{display:none}` below 1120px is overridden rather than
   edited in each of the seven files that carry a copy of it.
   The button itself is created by assets/js/nav.js.
   -------------------------------------------------------------------------- */
.nav .brand{position:relative; z-index:2}
.nav__burger{
  display:none; position:relative; z-index:2; flex:none;
  width:46px; height:46px; margin-left:10px; padding:0;
  align-items:center; justify-content:center; flex-direction:column; gap:5px;
  border:1px solid rgba(238,156,192,.28); border-radius:14px;
  background:rgba(44,10,27,.42); -webkit-backdrop-filter:blur(10px); backdrop-filter:blur(10px);
  cursor:pointer;
}
.nav__burger span{display:block; width:19px; height:1.6px; border-radius:2px; background:var(--cream);
  transition:transform .38s var(--ease), opacity .25s var(--ease)}
html.nav-open .nav__burger span:nth-child(1){transform:translateY(6.6px) rotate(45deg)}
html.nav-open .nav__burger span:nth-child(2){opacity:0}
html.nav-open .nav__burger span:nth-child(3){transform:translateY(-6.6px) rotate(-45deg)}
@media (max-width:1120px){
  .nav__burger{display:flex}
  .nav__links{
    position:fixed; z-index:1; left:0; right:0; top:0;
    display:flex; flex-direction:column; gap:0; margin:0;
    padding:calc(78px + env(safe-area-inset-top)) var(--gut) 30px;
    font-size:19px; color:var(--cream);
    background:rgba(28,7,17,.975); -webkit-backdrop-filter:blur(18px); backdrop-filter:blur(18px);
    border-bottom:1px solid rgba(238,156,192,.18);
    transform:translateY(-101%); transition:transform .46s var(--ease-morph);
    max-height:100dvh; overflow-y:auto; overscroll-behavior:contain;
  }
  .nav__links a{padding:15px 2px; border-bottom:1px solid rgba(238,156,192,.13)}
  .nav__links a:last-child{border-bottom:0}
  .nav__links a::after{display:none}      /* a hover underline means nothing on touch */
  html.nav-open .nav__links{transform:none}
  html.nav-open, html.nav-open body{overflow:hidden}
}
/* ------------------------------------------------------- touch targets ----
   Measured at 375px: the contents list was 20px tall, the breadcrumbs 22px
   and the links under the booking card 23px. Anything you tap with a thumb
   gets padded out to about 44px.
   -------------------------------------------------------------------------- */
@media (pointer:coarse), (max-width:760px){
  .toc{gap:0}
  .toc a{display:block; padding:12px 0; border-bottom:1px solid var(--stone)}
  .toc li:last-child a{border-bottom:0}
  .crumbs ol{gap:2px 10px}
  .crumbs a{display:inline-block; padding:10px 0}
  .crumbs li + li::before{margin-top:1px}
  .card--book .alt a{display:block; padding:11px 0}
  .chips a{padding:11px 16px}
  .faq summary{padding:18px 2px}
  .ft__col a{display:inline-block; padding:10px 0}
  .cta__num a, .ft__contact a{display:inline-block; padding:7px 0}
  /* These keep their look — an underline sitting right under the words — so
     the tap area is grown behind them instead of padding them apart. */
  .cd__more, .tx__more, .dr__more, .pf__link, .hw__link, .ft__dir,
  .cd__name a, .nav .brand{position:relative}
  .cd__more::before, .tx__more::before, .dr__more::before, .pf__link::before,
  .hw__link::before, .ft__dir::before, .cd__name a::before, .nav .brand::before{
    content:""; position:absolute; inset:-11px -6px;
  }
  /* the contact details — each on its own line, so padding is safe here */
  .visit .card p a, .wh__dl dd a{display:inline-block; padding:10px 0}
}
"""

# ============================================================================ Apache
# Old address -> new one. The site also ships a small redirect page at each old
# address (for hosts without .htaccess); on Apache these 301s answer first.
REDIRECTS = {
    'Halcyon-About.html': 'about-us/', 'Halcyon-Treatments.html': 'services/',
    'Halcyon-Conditions.html': 'pain-conditions/', 'Halcyon-Consultants.html': 'our-doctors/',
    'Halcyon-Privacy.html': 'privacy-policy/', 'shoulder-pain.html': 'shoulder-pain/',
    'request-an-appointment/': 'contact/', 'book-an-appointment/': 'contact/',
    'our-practice/': 'about-us/', 'for-patients/': '#jr', 'pain-conditions-2/': 'pain-conditions/',
    'pediatric-care/': '', 'best-pain-management-center-in-hyderabad/': '',
    'pain-relief-ad1/': '', 'pain-relief-ad2/': '', 'thankyou/': '', 'elementor-991776/': '',
    '8585072072-2/': 'contact/', 'dashboard/': '',
    'recognition-and-treatment-of-arthritis/': 'knee-pain-treatment/',
    'the-ultimate-guide-to-recognizing-and-treating-arthritis-symptoms/': 'knee-pain-treatment/',
    'natural-home-remedies-to-relieve-knee-pain-fast/': 'knee-pain-treatment/',
    'break-through-pain-with-cooled-radiofrequency/': 'radiofrequency-ablations/',
}
...
def write_htaccess():
    ...
    for old, new in sorted(REDIRECTS.items()):
        ...
        dst = U + (new[1:] if new.startswith('#') else new) if not new.startswith('#') else U + new
        rules.append('RedirectMatch 301 "^%s$" "%s"' % (src, dst))
    body = '\n'.join(rules)
    crm = SITE.get('crm_url', '')
    write('.htaccess', f"""# Halcyon — Apache configuration (GoDaddy cPanel / any Apache host).
# Generated by tools/build.py. Upload it with the site; it must sit in the web root.
...
<IfModule mod_rewrite.c>
  ...
  # nothing WordPress is here any more
  RewriteRule ^(wp-admin|wp-login\.php|wp-content|wp-includes|xmlrpc\.php|feed|comments/feed) - [R=410,L]
</IfModule>
...
# ---------------------------------------------------------------- staff
# A short address the callers can remember and keep on a phone's home screen.
# The CRM itself runs on Apps Script — its server half needs Apps Script's own
# access to the leads sheet, so it cannot be served from here. This is a doorway
# and nothing more.
#
# 302, not 301: a 301 is cached by the browser more or less for ever, and the
# target changes if that deployment is ever recreated. A stale 301 would strand
# every caller on a dead URL with no way to clear it but wiping their history.
RedirectMatch 302 "^/crm/?$" "{crm}"

# The address is guessable, so keep it out of search results. This says nothing
# about who may open it — that is the password on the Users sheet.
<IfModule mod_setenvif.c>
  SetEnvIf Request_URI "^/crm/?$" NOINDEX_PATH
</IfModule>
<IfModule mod_headers.c>
  Header always set X-Robots-Tag "noindex, nofollow" env=NOINDEX_PATH
</IfModule>
...
<IfModule mod_expires.c>
  ExpiresActive On
  # assets carry a ?v= stamp and are replaced, never edited
  ...
  # pages change: let the browser re-check every time
...
def build_assistant_kb():
    ...
    # Telugu layer: phrasings, key phrases and a one-line Telugu gist per entry, plus the
    # word list that lets the matcher recognise Telugu/Hindi typed in English letters
    patches, indic, ui, syn_extra = {}, {}, {}, {}
    for f in sorted(glob.glob('tools/content/assistant.te-*.json')):
        d = json.load(open(f))
        for eid, patch in (d.get('patches') or {}).items():
            p0 = patches.setdefault(eid, {'q': [], 'k': [], 'te': ''})
            p0['q'] += patch.get('q', []); p0['k'] += patch.get('k', [])
            p0['te'] = patch.get('te') or p0['te']
        indic.update(d.get('indic') or {})
        syn_extra.update(d.get('synonyms') or {})
        ui.update(d.get('ui') or {})
    missing = [eid for eid in patches if eid not in {e['id'] for e in entries}]
    if missing:
        print('  ! Telugu patch for unknown entries:', ', '.join(missing[:8]))
    for e in entries:
        p0 = patches.get(e['id'])
        if not p0:
            continue
        e['q'] = list(dict.fromkeys(e.get('q', []) + p0['q']))
        e['k'] = list(dict.fromkeys(e.get('k', []) + p0['k']))
        if p0['te']:
            e['te'] = p0['te']
    ...
    for kind, data, order in (('condition', CONDITIONS, COND_ORDER), ('treatment', TREATMENTS, TREAT_ORDER), ('doctor', DOCTORS, DOC_ORDER)):
        for slug in order:
            ...
            for i, f in enumerate(d.get('faqs', [])):
                ...
                if eid in have:
                    continue
                entries.append({'id': eid, 'q': [f['q']], 'k': [], 'kt': topic, 'faq': 1, 'a': f['a'], 'link': link,
    ...
    kb = {
        ...
        'synonyms': dict(core.get('synonyms', {}), **dict(syn_extra, **indic)),
        'indic': sorted(indic),
        'ui': ui,
    }
    ...
    engine = read('assets/js/assistant.js') + read('assets/css/assistant.css') + read('assets/js/analytics.js')
...
NAV_V = ''
NAV_A = '/* ===== halcyon mobile nav: generated by tools/build.py, do not hand-edit ===== */'
NAV_B = '/* ===== end halcyon mobile nav ===== */'


def mobile_nav(s, path):
    """Put the menu styles and script on the page.

    The built pages are committed, so a previous build's block is already
    sitting in the file. It is stripped and rewritten every time rather than
    skipped, or an edit to MOBILE_NAV_CSS would never reach the output.
    """
    s = re.sub(re.escape(NAV_A) + '.*?' + re.escape(NAV_B), '', s, flags=re.S)
    s = re.sub(r'\s*<script src="[^"]*assets/js/nav\.js[^"]*"[^>]*></script>', '', s)
    i = s.find('.nav__links{')
    if i > 0:
        j = s.index('</style>', i)
        s = s[:j] + NAV_A + MOBILE_NAV_CSS + NAV_B + s[j:]
    pre = '../' * path.count('/')
    return s.replace('</head>',
                     '  <script src="%sassets/js/nav.js?v=%s" defer></script>\n</head>' % (pre, NAV_V), 1)
...
def main():
    global NAV_V
    import hashlib
    NAV_V = hashlib.sha1(read('assets/js/nav.js').encode()).hexdigest()[:8]
    ...
    for p in PAGES:
        ...
        s = mobile_nav(footer_links(fix_images(s, p['path']), p['path']), p['path'])
        write(p['file'], s)
    ...
    print('built %d pages; sitemap has %d URLs; assistant knows %d entries, %d synonyms, '
          '%d Telugu answers, %d Telugu/Hindi words'
          % (len(PAGES), sum(1 for p in PAGES if p['kind'] != 'noindex'), n_kb[0], n_kb[1], n_kb[2], n_kb[3]))
